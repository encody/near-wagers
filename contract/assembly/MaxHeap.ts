...
export class MaxHeap<T> {
  ...
  private _siftDown(current: i32, currentValue: T): void {
    ...
    if (
      ...
      (leftValue = this._heap[left]) > currentValue
    ) {
      largest = left;
    ...
    if (
      ...
      (rightValue = this._heap[right]) > largestValue
    ) {
      largest = right;
  ...
  /**
   * Finds the index of an item in the heap using `===`
   *
   * @example
   * ```ts
   * const maxHeap = new MaxHeap<u64>('h');
   ...
   *
   *   @operator(">")
   *   private __gt(right: A): bool {
   *     return this.value > right.value;
   *   }
   * }
   *
   * const aHeap = new MaxHeap<A>('a');
   * const obj = new A(1);
   * aHeap.push(obj);
   * aHeap.indexOf(obj) == 0;
   * aHeap.indexOf(new A(1)) == -1;
   * ```
   *
   * @param target Item to search for
   ...
   * const obj = new A(1);
   * aHeap.push(obj);
   * aHeap.valueIndexOf(obj) == 0;
   * aHeap.valueIndexOf(new A(1)) == 0;
   * ```
   *
   * @param target Item to search for
   * @returns The index of the item in the heap, -1 otherwise
   */
  public valueIndexOf(target: T): i32 {
    if (this._heap.length == 0) {
      return -1;
    }
